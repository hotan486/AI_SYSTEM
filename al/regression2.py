...
loss=tf.reduce_mean(tf.square(y-y_h))
# 옵티마이저 정의
lr=0.1
...
#session run
for i in range(100):
    res=sess.run(train_step,feed_dict={x_h:x_data,y_h:y_data})
    #해당결과 빼오기
    #print("y 결과는:",res)
    w_h=sess.run(w_)
    b_h=sess.run(b_)
    loss_h=sess.run(loss,feed_dict={x_h:x_data,y_h:y_data})
    print("step:",i,"loss:",loss_h,",w_ :",w_h,"b_:",b_h)
    #print("b_ :",b_h)
    y_min=w_h*x_min+b_h
    y_max=w_h*x_max+b_h
    y_test=[y_min,y_max]
